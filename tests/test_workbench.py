...
class AnalysisEngineTests(unittest.TestCase):
    ...
    def test_windows_qa_reuses_one_hash_verified_candidate(self) -> None:
        ...
        self.assertIn("ProcessObject = $nativeProcess", lifecycle)
        self.assertIn("$confirmedPath -ine $capturedPath", lifecycle)
        self.assertIn("$confirmed.ParentProcessId -ne", lifecycle)
        self.assertIn("Wait-OwnedProcessExit", lifecycle)
        self.assertIn("-AllowAlreadyExited", lifecycle)
        self.assertIn("-NotBeforeUtc ([DateTime]$firstDesktopRecord.CreationDate)", lifecycle)
        self.assertIn("-NotBeforeUtc ([DateTime]$secondDesktopRecord.CreationDate)", lifecycle)
        self.assertIn("$owned.Kill()", lifecycle)
        self.assertNotIn("Stop-Process -Id ([uint32]$Record.Id)", lifecycle)
        self.assertNotIn("$owned.Kill($true)", lifecycle)
        self.assertEqual(lifecycle.count("Assert-OwnedProcessIdentity -Record $firstBackendRecord"), 1)
        self.assertEqual(lifecycle.count("Assert-OwnedProcessIdentity -Record $secondBackendRecord"), 1)
        ...
        self.assertEqual(hosted_smoke.count("-RequireExactAlive | Out-Null"), 1)
        self.assertIn("ProcessObject = $nativeProcess", hosted_smoke)
        self.assertIn("$confirmedPath -ine $capturedPath", hosted_smoke)
        self.assertIn("$confirmed.ParentProcessId -ne", hosted_smoke)
        self.assertIn("Wait-OwnedProcessExit", hosted_smoke)
        self.assertIn("$owned.Kill()", hosted_smoke)
        self.assertNotIn("Stop-Process -Id ([uint32]$Record.Id)", hosted_smoke)
        self.assertNotIn("$owned.Kill($true)", hosted_smoke)
    ...
    def test_store_identity_legal_layout_and_deletion_scope_are_explicit(self) -> None:
        ...
        self.assertIn("<DebugSymbols>false</DebugSymbols>", desktop_project)
        self.assertIn("<DebugType>none</DebugType>", desktop_project)
        self.assertIn('if ($Configuration -eq "Release")', build_windows)
        self.assertIn("Release layout contains forbidden PDB debug symbols", build_windows)
        self.assertIn("MSIX layout contains forbidden PDB debug symbols", build_msix)
